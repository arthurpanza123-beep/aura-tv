...
interface IptvCategory { category_id: string; category_name: string; }
interface IptvChannel {
  num: number; name: string; stream_id: number; stream_icon: string;
  category_id: string; epg_channel_id: string | null; tv_archive: number;
  stream_url?: string;
}
...
function ChannelsPage() {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      else if (e.key === "ArrowDown") { e.preventDefault(); setSelectedIndex(i => Math.min(filtered.length - 1, i + 1)); }
      else if (e.key === "Enter") { if (selected) handleWatch(selected); }
      else if (e.key === "Escape") { setShowOverlay(o => !o); }
    };
  ...
  const handleWatch = async (channel: IptvChannel) => {
    ...
    setShowPlayer(true);
    try {
      const result = await getStreamUrlFn({ data: { ...creds, streamId: channel.stream_id, type: "live" as const } });
    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center" style={{ background: "#0a1628" }}>
        <div className="text-center">
          ...
          <button onClick={() => navigate({ to: "/" })} className="px-6 py-2 bg-[#2a9af0] text-white rounded-lg cursor-pointer font-semibold">
            Fazer Login
          </button>
        </div>
      </div>
      ...
      <div className="flex h-screen w-screen overflow-hidden" style={{ background: "#0a1628" }}>
        ...
        <main className="flex-1 ml-16 flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={() => window.location.reload()} className="px-6 py-2.5 bg-[#2a9af0] text-white rounded-lg cursor-pointer font-semibold">
              Tentar Novamente
            </button>
          </div>
        </main>
  ...
  if (showPlayer) {
    return (
      <TVPlayer streamUrl={playerStreamUrl} title={playerTitle} subtitle="Canal ao Vivo"
      ...
      <main className="flex-1 ml-16 relative">
          ...
          {selected ? (
            <div className="text-center animate-in fade-in duration-300">
              {selected.stream_icon ? (
                <img src={selected.stream_icon} alt={selected.name}
              ...
              <p className="text-[#2a9af0]/50 text-sm mt-2 font-medium">{catMap.get(selected.category_id) || ""}</p>
              <div className="mt-6">
                <button onClick={() => handleWatch(selected)}
        ...
        {selected && (
          <div className="absolute top-4 left-6 z-20 flex items-center gap-3 bg-black/40 backdrop-blur-md rounded-xl px-4 py-2.5">
            {selected.stream_icon && (
              <img src={selected.stream_icon} alt="" className="w-8 h-6 object-contain rounded" loading="lazy"
                onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
            )}
            <div>
        ...
        {showOverlay && (
          <div className="absolute right-0 top-0 bottom-0 w-80 z-30 flex flex-col backdrop-blur-sm"
            style={{ background: "linear-gradient(270deg, rgba(6,10,22,0.97) 0%, rgba(6,10,22,0.85) 70%, transparent 100%)" }}>
            {/* Search */}
            ...
            <div className="flex flex-wrap gap-1.5 px-4 pb-2 max-h-28 overflow-y-auto no-scrollbar">
              <button onClick={() => { setActiveCategory("all"); setSelectedIndex(0); setVisibleCount(PAGE_SIZE); }}
                className={`px-3 py-1 rounded-full text-[10px] font-semibold cursor-pointer transition-all border ${
                  activeCategory === "all" ? "bg-[#2a9af0]/20 text-[#2a9af0] border-[#2a9af0]/40" : "bg-transparent text-[#6b7f99] border-[#1a2e48]/50 hover:text-white"
                }`}>
              ...
              {categories.map(cat => (
                <button key={cat.category_id}
                  ...
                  className={`px-3 py-1 rounded-full text-[10px] font-semibold cursor-pointer transition-all border ${
                    activeCategory === cat.category_id ? "bg-[#2a9af0]/20 text-[#2a9af0] border-[#2a9af0]/40" : "bg-transparent text-[#6b7f99] border-[#1a2e48]/50 hover:text-white"
                  }`}>
                  {cat.category_name}
                </button>
              ))}
            ...
            <p className="px-4 text-[10px] text-[#4a5a70] mb-1 font-medium">{filtered.length} canais encontrados</p>

            {/* Channel list */}
            <div ref={listRef} className="flex-1 overflow-y-auto px-2 space-y-0.5 no-scrollbar"
              onScroll={(e) => {
              ...
              {visibleChannels.map((channel, i) => (
                <button key={`${channel.stream_id}-${i}`}
                  ...
                  <div className={`w-10 h-7 rounded bg-gradient-to-br from-[#1a2e48] to-[#0f1e35] items-center justify-center shrink-0 ${channel.stream_icon ? 'hidden' : 'flex'}`}>
                    <span className="text-[7px] font-black text-[#4a6a8a] uppercase">{channel.name.slice(0, 3)}</span>
                  </div>
                  ...
                  {i === selectedIndex && (
                    <button onClick={(e) => { e.stopPropagation(); handleWatch(channel); }}
