...
export const Route = createFileRoute("/series")({
  head: () => ({
    meta: [
      { title: "Series - Central Play Plus" },
      { name: "description", content: "Catalogo de series do Central Play Plus" },
    ],
  }),
...
function SeriesPage() {
  ...
  const fetchSeriesCategories = useServerFn(fetchSeriesCategoriesFn);
  const fetchSeries = useServerFn(fetchSeriesFn);
    ...
    async function loadData() {
      try {
        const [cats, seriesList] = await Promise.all([
          fetchSeriesCategories({ data: creds! }),
          fetchSeries({ data: { ...creds! } }),
        ]);
        if (cancelled) return;
        ...
        if (!cancelled) setError("Erro ao carregar series.");
      } finally {
  ...
  }, [creds, fetchSeriesCategories, fetchSeries]);
  ...
  const handleOpenDetails = async (item: SeriesItem) => {
    ...
    setEpisodes([]);
    try {
      const result = await fetchSeriesInfo({ data: { ...creds, seriesId: item.series_id } });
      const parsed = JSON.parse(result.info);
      const eps: EpisodeItem[] = [];
      if (parsed.episodes) {
        for (const key of Object.keys(parsed.episodes)) {
          const seasonEps = parsed.episodes[key];
          if (Array.isArray(seasonEps)) {
            for (const ep of seasonEps) {
              eps.push({
                id: String(ep.id || ""),
                title: ep.title || ep.name || `Episodio ${eps.length + 1}`,
      ...
      setEpisodes(eps.filter((ep) => ep.id));
    } catch (err) {
  ...
  const openPlayer = async (contentId: string, title: string) => {
    if (!creds) return;
    setPlayerTitle(title);
    setShowPlayer(true);
    try {
      const result = await getPlaybackUrl({
        data: { ...creds, contentId, type: "series" as const },
      });
    ...
    return (
      ...
      >
        <div className="text-center">
          <p className="text-[#6b7f99] mb-4">Sessao expirada</p>
          <button
            onClick={() => navigate({ to: "/" })}
            ...
            <div className="flex gap-8 items-start">
              {selectedSeries.cover ? (
                <img
                  ...
                  className="w-48 rounded-2xl shadow-2xl border border-[#1a2e48] shrink-0"
                  loading="lazy"
                  onError={(e) => {
              ...
              <div className="flex-1 pt-4">
                ...
                <div className="flex items-center gap-4 mb-4 text-sm">
                  ...
                  <span className="text-[#6b7f99]">
                    {selectedSeries.genre || catMap.get(selectedSeries.category_id) || ""}
                  </span>
                  {selectedSeries.releaseDate && (
                    <span className="text-[#6b7f99]">{selectedSeries.releaseDate.slice(0, 4)}</span>
                  )}
                </div>
                {selectedSeries.plot && (
                  <p className="text-[#8a9bb5] text-sm leading-relaxed max-w-2xl mb-4">
                    {selectedSeries.plot}
                  </p>
                )}
              </div>
            ...
            {detailLoading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                {episodes.map((ep, i) => (
                  <button
                    key={ep.id}
                    onClick={() => openPlayer(ep.id, `${selectedSeries.name} - ${ep.title}`)}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[#6b7f99] text-xs">Episodio {i + 1}</p>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-[#6b7f99] mb-4">Nenhum episodio disponivel</p>
                <button
                  onClick={() => openPlayer(selectedSeries.id, selectedSeries.name)}
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl font-black text-[#e8edf4]">Series</h1>
              <p className="text-[#4a5a70] text-xs mt-0.5">
                Mostrando {visibleSeries.length} de {filtered.length} titulos
              </p>
            </div>
        ...
        <div className="px-4 pb-8">
          <div className="grid grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8 gap-3 px-4">
            {visibleSeries.map((s) => (
              <SeriesCard
                key={s.id}
                item={s}
                catName={catMap.get(s.category_id) || ""}
          ...
          {filtered.length === 0 && !loading && (
            <div className="py-20 text-center">
              ...
              <p className="text-[#6b7f99] text-lg">Nenhuma serie encontrada</p>
            </div>
          )}
        </div>
      </main>
