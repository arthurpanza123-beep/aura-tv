...
import { fetchSeriesCategoriesFn, fetchSeriesFn } from "@/server/iptv.functions";
import { Search, Clapperboard, AlertTriangle, Info } from "lucide-react";
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden" style={{ background: "linear-gradient(160deg, #0a1628 0%, #0d1f3c 30%, #0c1a2e 100%)" }}>
      ...
      >
        ...
        <p className="px-8 text-xs text-[#4a5a70] mb-3">{filtered.length} séries encontradas</p>

        {/* Grid */}
        <div className="px-4 pb-8">
          <div className="grid grid-cols-5 xl:grid-cols-7 2xl:grid-cols-8 gap-3 px-4">
            {visibleSeries.map((s) => (
              <SeriesCard key={s.series_id} item={s} catName={catMap.get(s.category_id) || ""} />
            ))}
          </div>
...
function SeriesCard({ item, catName }: { item: SeriesItem; catName: string }) {
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
        ...
        <div className="flex items-center gap-1.5 text-[10px] mb-2">
          {item.rating_5based > 0 && (
            <span className="text-yellow-400 font-semibold">★ {item.rating_5based.toFixed(1)}</span>
          )}
          <span className="text-[#8a9bb5]">{catName}</span>
          {item.releaseDate && <span className="text-[#8a9bb5]">{item.releaseDate.slice(0, 4)}</span>}
        </div>
        <button
          onClick={(e) => { e.stopPropagation(); navigate({ to: "/details/$id", params: { id: String(item.series_id) } }); }}
